...
'''
'''
假设能力值为 x 的员工有n个（即员工 x1, x2, ..., xn 的能力值均为x）n >= 1
由于 x1, x2, ..., xn 的排列服从均匀分布，此时 x1 在小 A 和小 B 的浏览顺序中出现在同一位置的概率为 1/n。
则这 n 个员工的简历在小 A 和小 B 的浏览顺序中出现在同一位置的份数的期望为 E = n · 1/n = 1。

所以：scores 数组内，在小 A 和小 B 的浏览顺序中出现在同一位置的简历数
等于 scores 数组内不同能力值的个数。
'''
