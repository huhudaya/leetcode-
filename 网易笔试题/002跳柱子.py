...
public class Main{
    public static void main(String []args){
        ...
        { 
            ...
            // 定义jump[i][j]为第i天，最多有j次机会进行超能力跳跃。选择是使用超能力还是不适用超能力
            ...
            for(int i = 0; i < n; i++)
            {
                for(int j = 1; j <= k && j + i < n; j++)
                {
                    ...
                    {//更新状态跟出发柱子相同
                        jump[j + i][0] |= jump[i][0];//表示剩余次数是0，同时本次不需要使用超能力，这个时候需要看上次剩余次数为0的情况
                        jump[j + i][1] |= jump[i][1];//表示剩余次数是1，同时本次不需要使用超能力，需要看上次剩余次数为1的情况
                    }
                    // 否则就需要使用超能力，这个时候前一个柱子就不能使用超能力，所以更新状态
                    else {
                        // 当本次选择需要使用超能力的时候，需要上次没有使用超能力，同时本次的jump[j + i][0]肯定是False
