...
14
思路:将一个数差分成若干个数，很显然其中某些数可能会出现多次
     就是说这些数可以用无限多次来构成最初的数
这就是完全背包问题：有n个物品，每个物品可以选无限多次，求选出的若干个物品的价值之和恰好为n的方案个数。
...
因此，总的方案数为f[i,j]=f[i-1,j]+(f[i,j-vi]+f[i,j-2vi]+...)
但是这样就要再枚举每件物品选的次数，时间复杂度变成o
...
所以每次当前状态i与i这一状态有关
所以只要在循环体积时，从小到大枚举，就可以保证每次状态都是新的,恰好达到统计效果。
